...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, agent skill files, and the `fm-top` cockpit (`fm-top.py`, `fm-top-poll.sh`) - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
...
tests/fm-gate-start.test.sh               # reliable gate-run starter: PWD=. poisoning immunity at the gate hook, stale-ref clear-before-push ordering, base64 intent push-option fidelity, scratch intent-file exclusion so teardown stays clean, refusal paths, and confirmation-timeout diagnostics
...
tests/fm-top.test.sh                      # fm-top gather() rows via --once-json: queued backlog, recent-done tail, unprobed background workflow rows, waiting/at-gate status labels, sort-direction toggling, and fleet-section grouping (rows cluster by source into crew/queued/workflow/done in fixed order)

...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as the edge-triggered PR-health poll (merge, conflict, or red-check transitions) and the failed-run reconciliation (a run whose outcome failed or was cancelled with no healthy delivered PR), terminal stale panes, non-terminal stale panes whose crew is not provably working, provably-working non-terminal stale panes that persist past `FM_STALE_ESCALATE_SECS` unless the crew is actively validating (an actively-running no-mistakes run is held indefinitely, never wedge-escalated), and heartbeat backstop hits.
...
The always-on watcher also uses that library's provably-working predicate on no-verb signal and non-terminal-stale paths, plus its narrower `crew_is_actively_validating` twin to exempt an actively-running no-mistakes run from wedge-escalation, while the daemon keeps its away-mode stale recheck unchanged.

...
FM_CHECK_INTERVAL=300   # seconds between slow checks (e.g. PR-health polls and failed-run reconciliation)
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working non-terminal stale pane escalates; an actively-validating no-mistakes run is exempt and held indefinitely; not-provably-working stale wakes surface immediately

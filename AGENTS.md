...
Because `fm-send` to a `kind=secondmate` target marks the request as from-firstmate (section 7 intake), its answer returns on the status/doc path, not in chat; read it there as an ordinary status signal.
...
The classifier lives in `bin/fm-classify-lib.sh` and is shared: the captain-relevant verb set and status-scan primitives back both this always-on watcher and the away-mode daemon, so the overlapping policy cannot drift; the provably-working predicate (`crew_is_provably_working`, reusing `bin/fm-crew-state.sh`) and its narrower wedge-exemption twin (`crew_is_actively_validating`, true only for an actively-running no-mistakes run step) live in that same library and run only on the watcher's no-verb / already-stale paths, never on every wake, so the per-wake triage stays cheap.
...
It also has the crewmate start the validation run itself with `bin/fm-gate-start.sh --intent-file <worktree file>` - a reliable direct gate push that carries the intent push option - and then drive the active run with `no-mistakes axi run`, which reattaches.
The helper exists because the `axi run` starter misfires on every fresh branch (it spawns its gate push with `PWD=.`, poisoning the gate hook's path on macOS `/bin/sh`) and the bare-push workaround silently drops the explicit intent; the root fix belongs upstream at `github.com/kunchenguid/no-mistakes`, and the helper stays a valid start path on a fixed tool too.

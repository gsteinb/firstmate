...
# needed. A missing or empty <seed_dir> is a silent, clean no-op (the common
# case). Best-effort by design: a copy failure warns to stderr and continues so a
# seed hiccup never blocks dispatch. Always returns 0.
